  ...
  <div class="container">
    ...
    <a-modal :title="!preview ? '材料填写' : '详情'" width="1300px" v-model:visible="visible" destroyOnClose>
      <a-form ref="form" :model="formState" :rules="formRules" :label-col="{ span: 8 }" :wrapper-col="{ span: 8 }">
        ...
        <a-form-item label="论文题目" v-bind="validateInfos.thesisTitle">
          <a-input v-model:value="formState.thesisTitle" placeholder="请填写论文题目" :disabled="preview" />
        </a-form-item>
        <a-form-item label="级别" v-bind="validateInfos.level">
          <a-input v-model:value="formState.level" placeholder="请填写级别" :disabled="preview" />
        </a-form-item>
        <a-form-item label="授奖单位或发表的期刊名称" v-bind="validateInfos.newspaperName">
          <a-input v-model:value="formState.newspaperName" placeholder="请填写授奖单位或发表的期刊名称" :disabled="preview" />
        </a-form-item>
        <a-form-item label="发表或获奖时间" v-bind="validateInfos.publishTime">
          <a-date-picker class="col-100" v-model:value="formState.publishTime" valueFormat="YYYY-MM-DD" placeholder="请填写发表或获奖时间" :disabled="preview" />
        </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const formRules = reactive({
      ...
      newspaperName: [{ required: true, message: '请输入报刊名称' }],
    });
    ...
    const resetValue = () => {
      ...
      formState.sno = '';
      formState.level = '';
      formState.thesisTitle = '';
      formState.newspaperName = '';
